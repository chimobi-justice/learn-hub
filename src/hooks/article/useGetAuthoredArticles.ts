...
import { GET_ARTHORED_ARTICLE_ENDPOINT } from '@api/index';

export const useGetAuthoredArticles = (limit: number = 0) => {
  const fetchPaginatedArticles = async ({ pageParam = 0 }) => {
    const res = await axiosInstance.get(`${GET_ARTHORED_ARTICLE_ENDPOINT}?limit=${limit}&page=${pageParam}`)
